...
st.title("🤰 산전검진휴가 실시간 판정 및 누적 관리 시스템 (내부망 전용)")
st.markdown("본 프로그램은 사내 PC 로컬 환경에서 안전하게 구동되며 외부에 데이터를 전송하지 않습니다.")
st.divider()
...
with col_right:
    ...
    with del_tab1:
        if not current_db.empty:
            options = {
                idx: f"[번호 {idx}] 사번: {row['사번']} | {row['성명']} | 신청일: {row['휴가신청일']}"
